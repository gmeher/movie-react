...
const MovieInfoMore = ({
  ...
  return (
    <div className='movie-more-info'>
      {vote_average ? (
        <>
          <h4 className='subject-heading'>TMDB User Score</h4>
          <p>
            {vote_average}/10
            <br />
            based on {numberWithCommas(vote_count)} users
          </p>
        </>
      ) : null}
      {release_date ? (
        <>
          <h4 className='subject-heading'>Release Date</h4>
          <p>{formatDate(release_date)}</p>
        </>
      ) : null}
      {budget ? (
        <>
          <h4 className='subject-heading'>Budget</h4>
          <p>{toUSD(budget)}</p>
        </>
      ) : null}
      {revenue ? (
        <>
          <h4 className='subject-heading'>Revenue</h4>
          <p>{toUSD(revenue)}</p>
        </>
      ) : null}
      {studios.length ? (
        <>
          ...
          {studios.map((element, iterator) => {
            return (
              <span key={iterator}>
                {element.name}
                {iterator < studios.length - 1 ? (
                  <span className='info-seperator'></span>
                ) : null}
              </span>
            );
          })}
          ...
          {spoken_languages.map((element, iterator) => {
            return (
              <span key={iterator}>
                {
                  <>
                    <span className='is-uppercase'>{element.iso_639_1} </span>
                    {element.name}
                  </>
                }
                {iterator < spoken_languages.length - 1 ? (
                  <span className='info-seperator'></span>
                ) : null}
              </span>
            );
