...
const MovieInfo = ({directors, movie, movieName, videos, writers}) => {
  var backDropImageLink = movie.backdrop_path
    ? `${IMAGE_BASE_URL}${BACKDROP_SIZE}${movie.backdrop_path}`
    : NoBackdrop;
  const backdropLoaded = () => {
    var vibrant = new Vibrant(backDropImageLink);
    vibrant.getPalette((err, palette) => {
      var backdropOverlayColor =
        'rgba(' + palette['DarkVibrant']['rgb'].toString() + ',.9);';
      document
        .querySelector('.movie-page-header-overlay')
        .setAttribute(
          'style',
          'background-color:' + backdropOverlayColor + ';',
        );
    });
  };
  ...
  const playTrailer = () => {
    var embedLink = document
      .querySelector('#buttonPlayTrailer')
  ...
  return (
    <div className='movie-page-header'>
      <img
        ...
        className='movie-page-header-backdrop'
        onLoad={backdropLoaded}
        alt={movie.title}
      />
      <div className='movie-page-header-overlay'></div>
      <Section>
        <Container>
                    ...
                    <p className='movie-quick-info'>
                      {movie.release_date ? (
                        <span className='movie-quick-info-span'>
                          <Link
                            to={{
                              pathname: `/year/${movie.release_date.slice(
                                0,
                                4,
                              )}`,
                            }}
                    ...
                    {directors.length ? (
                      <>
                        ...
                        {directors.map((element, iterator) => {
                          return (
                            <Link
                              to={{pathname: `/director/${element.id}`}}
                              key={iterator}
                              className='additional-link'>
                              {element.name}
                            </Link>
